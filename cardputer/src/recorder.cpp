...
static uint32_t  g_lastDataMs  = 0;   // für Dropout-Erkennung
...
// ── Mic-Konfig einmalig setzen und starten ────────────────────────────────────
...
void Recorder::tick() {
    ...
    // wait=false: korrekter Mono-16kHz-Pfad in M5Unified.
    // wait=true liefert bei diesem Board Stereo-Rohsamples (doppelt so viele
    // Bytes) → Audio klingt halb so schnell.
    if (!M5Cardputer.Mic.record(g_buf, REC_BUFFER_SIZE, SAMPLE_RATE, false)) {
        // Kein Buffer bereit – kurz yielden damit der Loop nicht mit
        // tausenden Leer-Polls den M5Unified-Zustand korrumpiert.
        delay(1);

        // Dropout-Erkennung: wenn >500 ms keine Daten, Mic neu starten
        if (millis() - g_lastDataMs > 500) {
            Serial.println("[Recorder] Dropout erkannt – Mic neu starten");
            M5Cardputer.Mic.end();
            delay(10);
            micBegin();
            g_lastDataMs = millis();
    ...
    if (written != bytes) {
        Serial.printf("[Recorder] FEHLER: write %u/%u – SD voll?\n",
                      written, bytes);
        g_writeError = true;
        return;
    }
